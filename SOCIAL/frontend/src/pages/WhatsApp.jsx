...
const WhatsAppAutomation = () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.whatsapp_automation?.config) {
          setConfig(prev => ({ ...prev, ...data.whatsapp_automation.config }));
        }
      }
    } catch (error) {
  ...
  const setupWhatsAppAutomation = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE}/whatsapp/setup`, {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        alert('WhatsApp automation setup successful!');
        fetchAutomationStatus();
  ...
  const generateAIMessage = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE}/ai/generate-whatsapp-content`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${user?.token}`
      ...
      if (result.success) {
        setMessageData(prev => ({ 
          ...prev, 
          message: result.message || result.content || 'AI generated message'
        }));
      } else {
  ...
  const sendBroadcast = async () => {
    ...
    if (recipients.length === 0 || !broadcastData.message) {
      alert('Please provide recipients and message');
      return;
    }

    setLoading(true);
    try {
      const response = await fetch(`${API_BASE}/whatsapp/send-broadcast`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${user?.token}`
      ...
      if (result.success) {
        const stats = result.broadcast_results;
        alert(`Broadcast sent! Success: ${stats.successful}, Failed: ${stats.failed}`);
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '40px', color: 'white' }}>
          ...
          <p style={{ fontSize: '20px', opacity: 0.9, maxWidth: '800px', margin: '0 auto' }}>
            Automate your WhatsApp Business messaging with AI-powered responses, broadcasts, and customer engagement
          </p>
        </div>
          ...
          <div style={{ background: 'rgba(255, 255, 255, 0.95)', borderRadius: '20px', padding: '40px', boxShadow: '0 8px 32px rgba(0, 0, 0, 0.2)' }}>
              ...
              <div>
                ...
                <div style={{ marginBottom: '20px' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setConfig(prev => ({ ...prev, auto_reply_enabled: e.target.checked }))}
                      style={{ width: '20px', height: '20px' }}
                    />
                    <span style={{ fontWeight: '600', color: '#333' }}>Enable Auto-Reply</span>
                  </label>
                </div>
                ...
                <div style={{ marginBottom: '20px' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer' }}>
                    <input
                      type="checkbox"
                      checked={config.campaign_enabled}
                      onChange={(e) => setConfig(prev => ({ ...prev, campaign_enabled: e.target.checked }))}
                      style={{ width: '20px', height: '20px' }}
                    />
                    <span style={{ fontWeight: '600', color: '#333' }}>Enable Campaigns</span>
                  </label>
                </div>
            ...
            <div style={{ textAlign: 'center', marginTop: '30px' }}>
              <button
                ...
                disabled={loading || !config.phone_number_id || !config.access_token}
                style={{
                  padding: '16px 32px',
                  background: loading || !config.phone_number_id || !config.access_token ? '#ccc' : '#25D366',
                  color: 'white',
                  border: 'none',
                  borderRadius: '12px',
                  fontSize: '16px',
                  fontWeight: '700',
                  cursor: loading || !config.phone_number_id || !config.access_token ? 'not-allowed' : 'pointer',
                  display: 'flex',
                  alignItems: 'center',
                  gap: '8px',
                  margin: '0 auto'
                ...
                <div style={{ background: '#f8f9fa', borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ fontWeight: '600', color: status.whatsapp_automation?.enabled ? '#00C851' : '#ff4444' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ fontWeight: '600', color: status.whatsapp_automation?.config?.auto_reply_enabled ? '#00C851' : '#666' }}>
                        Auto-Reply {status.whatsapp_automation?.config?.auto_reply_enabled ? 'On' : 'Off'}
                      </span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <span>{status.whatsapp_automation?.config?.campaign_enabled ? '✅' : '❌'}</span>
                      <span style={{ fontWeight: '600', color: status.whatsapp_automation?.config?.campaign_enabled ? '#00C851' : '#666' }}>
                        Campaigns {status.whatsapp_automation?.config?.campaign_enabled ? 'On' : 'Off'}
                      </span>
                    </div>
                  </div>
                </div>

                {status.whatsapp_automation?.config && (
                  <div style={{ background: '#f8f9fa', borderRadius: '12px', padding: '20px' }}>
                    <h3 style={{ color: '#128C7E', marginBottom: '16px' }}>Business Information</h3>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px' }}>
                      <div>
                        <strong>Business Name:</strong> {status.whatsapp_automation.config.business_name || 'Not set'}
                      </div>
                      <div>
                        <strong>Business Hours:</strong> {status.whatsapp_automation.config.business_hours?.start || '09:00'} - {status.whatsapp_automation.config.business_hours?.end || '18:00'}
                      </div>
                      <div>
                        <strong>Timezone:</strong> {status.whatsapp_automation.config.timezone || 'UTC'}
                      </div>
                      <div>
                        <strong>Last Activity:</strong> {status.whatsapp_automation.stats?.last_activity ? new Date(status.whatsapp_automation.stats.last_activity).toLocaleString() : 'None'}
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  <button
                    ...
                    style={{
                      padding: '12px 24px',
                      background: loading || !messageData.to || !messageData.message ? '#ccc' : '#25D366',
                      color: 'white',
                      border: 'none',
                      borderRadius: '8px',
                      cursor: loading || !messageData.to || !messageData.message ? 'not-allowed' : 'pointer',
                      fontWeight: '600',
                      flex: '1'
                    }}
                  >
              ...
              <div>
                ...
                <div style={{ marginBottom: '20px' }}>
                  ...
                  <select
                    ...
                    onChange={(e) => setMessageData(prev => ({ ...prev, message_type: e.target.value }))}
                    style={{
                      width: '100%',
                      padding: '12px 16px',
                      border: '2px solid #ddd',
                ...
                <div style={{ marginBottom: '20px' }}>
                  ...
                  <select
                    ...
                    onChange={(e) => setMessageData(prev => ({ ...prev, business_type: e.target.value }))}
                    style={{
                      width: '100%',
                      padding: '12px 16px',
                      border: '2px solid #ddd',
                ...
                <div style={{ background: '#f8f9fa', borderRadius: '8px', padding: '16px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <button
                      onClick={() => setMessageData(prev => ({ ...prev, message: 'Hi! Thank you for your interest in our services. How can we help you today?' }))}
                      style={{ padding: '8px 12px', background: 'white', border: '1px solid #ddd', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', textAlign: 'left' }}
                    >
                      Welcome Message
                    </button>
                    <button
                      onClick={() => setMessageData(prev => ({ ...prev, message: 'Thank you for your order! We will process it shortly and keep you updated.' }))}
                      style={{ padding: '8px 12px', background: 'white', border: '1px solid #ddd', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', textAlign: 'left' }}
                    >
                      Order Confirmation
                    </button>
                    <button
                      onClick={() => setMessageData(prev => ({ ...prev, message: 'We have a special offer just for you! Contact us to learn more.' }))}
                      style={{ padding: '8px 12px', background: 'white', border: '1px solid #ddd', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', textAlign: 'left' }}
                    >
                      Promotional Message
                    </button>
        ...
        {activeTab === 'broadcast' && (
          <div style={{ background: 'rgba(255, 255, 255, 0.95)', borderRadius: '20px', padding: '40px', boxShadow: '0 8px 32px rgba(0, 0, 0, 0.2)' }}>
            ...
            <div style={{ marginBottom: '30px' }}>
              <div style={{ background: '#fff3cd', border: '1px solid #ffeeba', borderRadius: '8px', padding: '16px', marginBottom: '20px' }}>
                <h4 style={{ color: '#856404', margin: '0 0 8px 0' }}>⚠️ Important Note</h4>
                ...
                <div>
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setBroadcastData(prev => ({ ...prev, recipients: e.target.value }))}
                ...
                <div>
                  ...
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    <select
                      ...
                      onChange={(e) => setBroadcastData(prev => ({ ...prev, media_type: e.target.value }))}
                      disabled={!broadcastData.media_url}
                      style={{
                        width: '100%',
                        padding: '12px 16px',
                        border: '2px solid #ddd',
                        ...
                        fontSize: '16px',
                        boxSizing: 'border-box',
                        opacity: !broadcastData.media_url ? 0.5 : 1
                      }}
                    >
                  ...
                  <div style={{ background: '#f8f9fa', borderRadius: '8px', padding: '16px' }}>
                    <h4 style={{ color: '#128C7E', marginBottom: '12px' }}>Broadcast Templates</h4>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <button
                        onClick={() => setBroadcastData(prev => ({ ...prev, message: 'Exciting news! We have a special promotion running this week. Don\'t miss out!' }))}
                        style={{ padding: '8px 12px', background: 'white', border: '1px solid #ddd', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', textAlign: 'left' }}
                      >
                        Promotion Announcement
                      </button>
                      <button
                        onClick={() => setBroadcastData(prev => ({ ...prev, message: 'Thank you for being our valued customer! We have some exciting updates to share with you.' }))}
                        style={{ padding: '8px 12px', background: 'white', border: '1px solid #ddd', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', textAlign: 'left' }}
                      >
                        Customer Appreciation
                      </button>
                      <button
                        onClick={() => setBroadcastData(prev => ({ ...prev, message: 'Important update: Our business hours have been updated. Please check our latest schedule.' }))}
                        style={{ padding: '8px 12px', background: 'white', border: '1px solid #ddd', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', textAlign: 'left' }}
                      >
                        Business Update
                      </button>
              ...
              <div style={{ textAlign: 'center', marginTop: '30px' }}>
                <button
                  ...
                  disabled={loading || !broadcastData.recipients || !broadcastData.message}
                  style={{
                    padding: '16px 32px',
                    background: loading || !broadcastData.recipients || !broadcastData.message ? '#ccc' : '#25D366',
                    color: 'white',
                    border: 'none',
                    borderRadius: '12px',
                    fontSize: '16px',
                    fontWeight: '700',
                    cursor: loading || !broadcastData.recipients || !broadcastData.message ? 'not-allowed' : 'pointer',
                    display: 'flex',
                    alignItems: 'center',
                    gap: '8px',
                    margin: '0 auto'
          ...
          <div style={{ background: 'rgba(255, 255, 255, 0.95)', borderRadius: '20px', padding: '40px', boxShadow: '0 8px 32px rgba(0, 0, 0, 0.2)' }}>
            ...
            {status && (
              <div>
                ...
                <div style={{ background: '#f8f9fa', borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
                  <h3 style={{ color: '#128C7E', marginBottom: '16px' }}>Activity Summary</h3>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px' }}>
                    <div>
                      <strong>Business Name:</strong> {status.whatsapp_automation?.config?.business_name || 'Not configured'}
                    </div>
                    <div>
                ...
                <div style={{ background: '#e8f5e8', borderRadius: '12px', padding: '20px' }}>
                  <h3 style={{ color: '#128C7E', marginBottom: '16px' }}>Performance Tips</h3>
                  <ul style={{ margin: 0, paddingLeft: '20px', color: '#333' }}>
                    <li style={{ marginBottom: '8px' }}>Personalize your messages to improve engagement rates</li>
                    <li style={{ marginBottom: '8px' }}>Send messages during business hours for better response rates</li>
                    <li style={{ marginBottom: '8px' }}>Use templates for consistent messaging</li>
                    <li style={{ marginBottom: '8px' }}>Monitor failed messages and update contact information</li>
                    <li>Keep messages concise and include clear calls-to-action</li>
                  </ul>
                </div>
              </div>
            )}
