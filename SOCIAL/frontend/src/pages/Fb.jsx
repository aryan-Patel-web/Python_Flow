...
const FacebookAutomation = () => {
  ...
  return (
    <div style={{ fontFamily: 'system-ui, -apple-system, sans-serif', minHeight: '100vh', background: 'linear-gradient(135deg, #4267B2 0%, #365899 100%)', display: 'flex' }}>
      ...
      {loading && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0, 0, 0, 0.7)', backdropFilter: 'blur(8px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999 }}>
          <div style={{ background: 'white', padding: '40px', borderRadius: '16px', textAlign: 'center', boxShadow: '0 15px 50px rgba(0, 0, 0, 0.3)' }}>
            <div style={{ border: '4px solid #f3f3f3', borderTop: '4px solid #4267B2', borderRadius: '50%', width: '50px', height: '50px', animation: 'spin 1s linear infinite', margin: '0 auto' }}></div>
            <p style={{ marginTop: '20px', color: '#666', fontSize: '16px', fontWeight: '600' }}>Processing Facebook request...</p>
          </div>
        </div>
      )}
      ...
      <div style={{ width: '320px', background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', height: '100vh', position: 'fixed', display: 'flex', flexDirection: 'column', boxShadow: '4px 0 25px rgba(0, 0, 0, 0.1)', border: '1px solid rgba(255, 255, 255, 0.2)' }}>
        ...
        <div style={{ padding: '40px 28px', borderBottom: '1px solid rgba(0, 0, 0, 0.1)', textAlign: 'center', background: 'linear-gradient(135deg, #4267B2, #365899)' }}>
          <h2 style={{ fontSize: '26px', fontWeight: '700', color: 'white', margin: 0, marginBottom: '6px' }}>Facebook Automation</h2>
          <div style={{ fontSize: '14px', color: 'rgba(255, 255, 255, 0.9)', fontWeight: '500' }}>AI-Powered Content & Posting</div>
        ...
        <nav style={{ flex: 1, padding: '28px 20px' }}>
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '16px', padding: '20px 24px', border: 'none', background: activeTab === tab.id ? 'linear-gradient(135deg, #4267B2, #365899)' : 'none', color: activeTab === tab.id ? 'white' : '#666', textAlign: 'left', borderRadius: '16px', cursor: 'pointer', marginBottom: '12px', fontSize: '16px', transform: activeTab === tab.id ? 'translateX(6px)' : 'none', boxShadow: activeTab === tab.id ? '0 6px 20px rgba(66, 103, 178, 0.4)' : 'none', transition: 'all 0.3s ease' }}>
        ...
        <div style={{ padding: '24px 28px', borderTop: '1px solid rgba(0, 0, 0, 0.05)' }}>
          <h3 style={{ fontSize: '16px', fontWeight: '700', color: '#374151', marginBottom: '16px' }}>Facebook Connection</h3>
          <div style={{ padding: '20px', background: 'rgba(255,255,255,0.1)', borderRadius: '16px', marginBottom: '16px', border: '2px solid #4267B230' }}>
            {facebookConnected ? (
              <div>
                ...
                {facebookUsername && <div style={{ fontSize: '14px', color: '#059669', margin: '0 0 12px 18px', background: 'rgba(34, 197, 94, 0.1)', padding: '6px 12px', borderRadius: '16px', display: 'inline-block', fontWeight: '600' }}>{facebookUsername}</div>}
                {facebookPages && facebookPages.length > 0 && (
                  <div style={{ marginBottom: '12px' }}>
                    <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '6px', fontWeight: '600' }}>Connected Pages ({facebookPages.length}):</div>
                    ...
                    {facebookPages.length > 2 && <div style={{ fontSize: '11px', color: '#6b7280', marginLeft: '18px' }}>+{facebookPages.length - 2} more pages</div>}
                  </div>
                )}
                <button onClick={testConnection} disabled={loading} style={{ width: '100%', padding: '10px 16px', fontSize: '14px', fontWeight: '700', background: loading ? '#d1d5db' : 'linear-gradient(135deg, #22c55e, #16a34a)', color: 'white', border: 'none', borderRadius: '10px', cursor: loading ? 'not-allowed' : 'pointer' }}>
              ...
              <div>
                ...
                <button onClick={handleConnect} disabled={loading} style={{ width: '100%', padding: '10px 16px', fontSize: '14px', fontWeight: '700', background: loading ? '#d1d5db' : 'linear-gradient(135deg, #4267B2, #365899)', color: 'white', border: 'none', borderRadius: '10px', cursor: loading ? 'not-allowed' : 'pointer' }}>
      ...
      <div style={{ flex: 1, marginLeft: '320px', background: 'rgba(255, 255, 255, 0.05)', backdropFilter: 'blur(10px)' }}>
        ...
        <header style={{ padding: '40px 48px', background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', borderBottom: '1px solid rgba(0, 0, 0, 0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h1 style={{ fontSize: '36px', fontWeight: '700', color: '#333', marginBottom: '8px' }}>Facebook Automation</h1>
            <p style={{ fontSize: '18px', color: '#666', margin: 0 }}>AI-powered Facebook automation with human-like content generation</p>
          </div>
          ...
          <div style={{ display: 'flex', gap: '16px' }}>
            {userProfile.isConfigured && (
              <button onClick={startAutomation} disabled={loading || !facebookConnected} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '16px 28px', border: 'none', borderRadius: '12px', fontSize: '16px', fontWeight: '700', cursor: loading || !facebookConnected ? 'not-allowed' : 'pointer', background: loading || !facebookConnected ? '#bdc3c7' : 'linear-gradient(135deg, #4267B2, #365899)', color: 'white', boxShadow: '0 6px 20px rgba(66, 103, 178, 0.4)', transition: 'all 0.3s ease' }}>
                <span>📘</span><span>Start Facebook Automation</span>
              </button>
            )}
          </div>
        </header>
        ...
        <div style={{ padding: '40px 48px' }}>
          <div style={{ background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', borderRadius: '24px', padding: '40px', boxShadow: '0 10px 40px rgba(0, 0, 0, 0.1)', border: '1px solid rgba(255, 255, 255, 0.2)', maxWidth: '1200px', margin: '0 auto' }}>
              ...
              <div>
                ...
                {userProfile.isConfigured && (
                  <div style={{ marginTop: '24px', padding: '20px', backgroundColor: 'rgba(34, 197, 94, 0.1)', borderRadius: '16px', border: '2px solid rgba(34, 197, 94, 0.3)' }}>
                    ...
                    <div style={{ color: '#155724', fontSize: '16px' }}>AI is ready for human-like content generation on Facebook.</div>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'schedule' && (
              <div>
                ...
                <div style={{ maxWidth: '600px', margin: '0 auto' }}>
                  <div style={{ padding: '32px', background: 'rgba(255,255,255,0.7)', borderRadius: '20px', border: '3px solid #4267B2' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px' }}>
                      <span style={{ fontSize: '40px' }}>📘</span>
                      <h3 style={{ margin: 0, color: '#4267B2', fontSize: '24px', fontWeight: '700' }}>Facebook Schedule</h3>
                    </div>
                    
                    <div style={{ marginBottom: '20px' }}>
                      <label style={{ display: 'block', fontSize: '16px', fontWeight: '700', marginBottom: '12px', color: '#374151' }}>Posts Per Day</label>
                      <input type="number" min="1" max="5" value={autoPostConfig.postsPerDay} onChange={(e) => setAutoPostConfig(prev => ({ ...prev, postsPerDay: parseInt(e.target.value) || 1 }))} style={{ padding: '12px 16px', border: '2px solid rgba(0, 0, 0, 0.1)', borderRadius: '10px', width: '100px', fontSize: '16px', fontWeight: '600' }} />
                      <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '6px' }}>Recommended: 2-3 posts per day</div>
                    </div>

                    <div style={{ marginBottom: '20px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                        <label style={{ fontSize: '16px', fontWeight: '700', color: '#374151' }}>Posting Times</label>
                        <button type="button" onClick={addTime} style={{ padding: '8px 16px', fontSize: '12px', background: '#4267B2', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>Add +5min</button>
                      </div>
                      
                      <input type="time" onChange={(e) => { if (e.target.value && !autoPostConfig.postingTimes.includes(e.target.value)) { setAutoPostConfig(prev => ({ ...prev, postingTimes: [...prev.postingTimes, e.target.value].sort() })); e.target.value = ''; } }} style={{ padding: '10px 12px', border: '2px solid rgba(0, 0, 0, 0.1)', borderRadius: '8px', fontSize: '14px', marginBottom: '12px', width: '100%' }} />

                      {autoPostConfig.postingTimes.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                          {autoPostConfig.postingTimes.map(time => (
                            <span key={time} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '6px 12px', background: 'white', border: '2px solid #4267B240', borderRadius: '16px', color: '#4267B2', fontSize: '13px', fontWeight: '600' }}>
                              {time}
                    ...
                    <div style={{ padding: '16px', backgroundColor: '#4267B215', borderRadius: '12px', marginTop: '20px' }}>
                      <h4 style={{ margin: '0 0 8px 0', color: '#4267B2', fontSize: '16px', fontWeight: '700' }}>Automation Status:</h4>
                      <div style={{ fontSize: '14px', color: '#374151', fontWeight: '500' }}>
                        {autoPostConfig.enabled ? (
                          <span style={{ color: '#22c55e' }}>✅ Active - {autoPostConfig.postsPerDay} posts/day</span>
                        ) : (
            ...
            {activeTab === 'status' && (
              <div>
                <h2 style={{ fontSize: '32px', fontWeight: '700', color: '#333', marginBottom: '40px', textAlign: 'center' }}>Facebook Analytics & Status</h2>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px', marginBottom: '40px' }}>
                  ...
                  <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 6px 20px rgba(0, 0, 0, 0.08)', border: '2px solid #4267B230' }}>
                    <h3 style={{ marginTop: 0, fontSize: '20px', fontWeight: '700', color: '#4267B2' }}>🤖 AI Profile Status</h3>
                  ...
                  <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 6px 20px rgba(0, 0, 0, 0.08)', border: '2px solid #4267B230' }}>
                    <h3 style={{ marginTop: 0, fontSize: '20px', fontWeight: '700', color: '#4267B2' }}>⚙️ Automation Status</h3>
                    <div style={{ padding: '8px 16px', borderRadius: '20px', fontSize: '14px', fontWeight: '700', backgroundColor: autoPostConfig.enabled ? '#d4edda' : '#f8d7da', color: autoPostConfig.enabled ? '#155724' : '#721c24', display: 'inline-block', marginBottom: '12px' }}>
                      {autoPostConfig.enabled ? 'Active' : 'Inactive'}
                    </div>
                    {autoPostConfig.enabled && (
                      <div style={{ fontSize: '14px', color: '#666' }}>
                        <p style={{ margin: '8px 0' }}>Posts/Day: <strong>{autoPostConfig.postsPerDay}</strong></p>
                        <p style={{ margin: '8px 0' }}>Times: <strong>{autoPostConfig.postingTimes.length}</strong></p>
                      </div>
                    )}
                  </div>
                </div>

                <div style={{ padding: '32px', backgroundColor: '#f8f9fa', borderRadius: '20px', border: '2px solid rgba(66, 103, 178, 0.2)' }}>
                  <h3 style={{ marginTop: 0, fontSize: '24px', fontWeight: '700', color: '#4267B2', textAlign: 'center', marginBottom: '32px' }}>📊 Performance Metrics</h3>
